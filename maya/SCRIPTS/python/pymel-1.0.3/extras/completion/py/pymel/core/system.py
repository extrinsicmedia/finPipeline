    ...
    def load(self, newFile=None, **kwargs):
        ...
        Flags:
          - loadNoReferences:
              ...
              references will be loaded. When used with -import, -reference or -loadReference flags,    will load the top-most
              reference only.
          - loadReferenceDepth:
              Used to specify which references should be loaded. Valid types are all, noneand topOnly, which will load all references,
              no references and top-level references only, respectively. Must be used with the -open,    -import, -reference or
              -loadReference flags. When noneis used with -loadReference, only path validation is performed. This can be used to
              replace a reference without triggering reload.
          - returnNewNodes:
...
def aaf2fcp(*args, **kwargs):
    ...
    Flags:
      ...
      - terminate : t                  (int)           [create]
          Complete the task                                         Flag can have multiple arguments, passed either as a tuple or
          a list.
...
def allNodeTypes(*args, **kwargs):
    ...
    Flags:
      - includeAbstract : ia           (bool)          [create]
          ...
          suffix (abstract)appended to them in the list.                                     Flag can have multiple arguments,
...
def assignInputDevice(*args, **kwargs):
    ...
    Flags:
      ...
      - immediate : im                 (bool)          [create]
          Immediately executes the command, without using the queue.                        Flag can have multiple arguments,
          passed either as a tuple or a list.
...
def attachDeviceAttr(*args, **kwargs):
    ...
    Flags:
      ...
      - cameraRotate : cr              (bool)          [create]
          This flag attaches the device/axis to the current cameras rotation controls.                      Flag can have multiple
...
def attrCompatibility(*args, **kwargs):
    """
    This command is used to handle compatibility issues between file format versions by providing a mechanism to describe
    differences between two versions.  Only plugin writers should have the potential need to use this command.The first
    optional command argument argument is a node name and the second optional command argument is an attribute
    name.Warning:Only use this command to describe changes in names or attributes of nodes that youhave written as plugins.
    Do notuse this command to change information about builtin dependency graph nodes.
    ...
    Flags:
      ...
      - version : v                    (unicode)       [create]
          Set the version target for subsequent commands to the given string.                       Flag can have multiple
          arguments, passed either as a tuple or a list.
...
def audioTrack(*args, **kwargs):
    ...
    Flags:
      ...
      - numTracks : nt                 (int)           [query]
          To query the number of audio tracks                                       Flag can have multiple arguments, passed
...
def cacheFileCombine(*args, **kwargs):
    ...
    Flags:
      ...
      - channelName : cnm              (unicode)       [edit]
          ...
          the first channel in the file is used.                                     Flag can have multiple arguments, passed
          either as a tuple or a list.
...
def cacheFileTrack(*args, **kwargs):
    ...
    Flags:
      ...
      - removeEmptyTracks : ret        (bool)          [create]
          This flag is used to remove all tracks that have no clips.                                        Flag can have multiple
          arguments, passed either as a tuple or a list.
...
def clearCache(*args, **kwargs):
    ...
    Flags:
      ...
      - dirty : d                      (bool)          [create]
          If toggled then remove all heavy data that is dirty.                                      Flag can have multiple
          arguments, passed either as a tuple or a list.
...
def convertUnit(*args, **kwargs):
    ...
    Flags:
      ...
      - toUnit : t                     (unicode)       [create]
          The unit to convert to.  If not supplied, it is assumed to be the system default                          Flag can have
...
def dagObjectCompare(*args, **kwargs):
    ...
    Flags:
      ...
      - short : s                      (bool)          [create]
          Compress output to short form (not as verbose)                                    Flag can have multiple arguments,
          passed either as a tuple or a list.
...
def date(*args, **kwargs):
    ...
    Flags:
      ...
      - format : f                     (unicode)       [create]
          ...
          if necessarymmCurrent minute, with leading 0 if necessaryssCurrent second, with leading 0 if necessaryFlag can have
          multiple arguments, passed either as a tuple or a list.
...
def dbmessage(*args, **kwargs):
    ...
    Flags:
      - file : f                       (unicode)       [create]
          ...
          window of Developer Studio. Default value is stdout.                                           Flag can have multiple
          arguments, passed either as a tuple or a list.
...
def detachDeviceAttr(*args, **kwargs):
    ...
    Flags:
      - all : all                      (bool)          [create]
          Delete all attachments on every device.                   Flag can have multiple arguments, passed either as a tuple or
          a list.
...
def dgInfo(*args, **kwargs):
    ...
    Flags:
      ...
      - outputFile : of                (unicode)       [create]
          Send the output to the file FILE instead of STDERR                                        Flag can have multiple
          arguments, passed either as a tuple or a list.
...
def dgdirty(*args, **kwargs):
    ...
    Flags:
      - allPlugs : a                   (bool)          [create]
          Ignore the selected or specified objects and dirty (or clean) all plugs.                          Flag can have multiple
          arguments, passed either as a tuple or a list.
...
def dgeval(*args, **kwargs):
    ...
    Flags:
      ...
      - verbose : v                    (bool)          [create]
          ...
          command line.                         Flag can have multiple arguments, passed either as a tuple or a list.
...
def dgtimer(*args, **kwargs):
    ...
    Flags:
      ...
      - outputFile : o                 (unicode)       [query]
          Used in conjunction with the -query flag to cause output to be directed to an output file. By default, output is to
          stdout, but can be redirected e.g. -outputFile /home/virginia/timing/dgtrace.txt. This makes it handy to load the data
          into an external application such as a spreadsheet program. To force output to the script editor, use -outputFile
          MELwhich makes it easy to grab the values from within a MEL script.
      ...
      - rangeUpper : rgu               (float)         [create]
          ...
          above the lower range limit are considered.                                           Flag can have multiple arguments,
          passed either as a tuple or a list.
      ...
      - trace : tr                     (bool)          [create]
          Turns on or off detailed execution tracing to the terminal window (stderr on Linux and Macintosh), and the status window
          on Windows. By default, tracing is off. If enabled, each timeable operation is logged when it starts and again when it
          ends. The following example shows how the output is formatted:dgtimer:begin: compute 3 particleShape1Deformed
      ...
      - type : t                       (unicode)       [create,query]
          ...
          entry, and only one line of output is generated (i.e. the global timers and header information is omitted). Note that
          one can force output to the script editor window via the -outputFile MELoption to make it easy to grab the values in a
          MEL script. Note: the -name and -type flag cannot be used together.
...
def error(*args, **kwargs):
    ...
    Flags:
      - showLineNumber : sl            (bool)          [create]
          ...
          display instead.                                    Flag can have multiple arguments, passed either as a tuple or a
          list.
...
def exportEdits(*args, **kwargs):
    ...
    Flags:
      - editCommand : ec               (unicode)       [create,query]
          ...
          connectAttr, deleteAttr, disconnectAttr, parent, and setAttr. In some contexts, this flag may be specified more than
      ...
      - exportSelected : exs           (bool)          [create,query]
          ...
          exported nodes will be exported.
      ...
      - type : typ                     (unicode)       [create,query]
          Set the type of the exported file. Valid values are mayaAsciior mayaBinary. Note that this command respects the global
...
def fileBrowserDialog(*args, **kwargs):
    """
    The fileBrowserDialog command offers more control of the windows file browser dialog. See the fileDialog command for
    ...
    Flags:
      ...
      - fileCommand : fc               (callable)      [create]
      ...
      - windowTitle : wt               (unicode)       [create]
          Set the window title of a style 1 or 2 dialog box                         Flag can have multiple arguments, passed
          either as a tuple or a list.
...
def fileDialog(*args, **kwargs):
    """
    Displays a file picking window, and returns the name of the file that the user picked.  If the user picked no file, then
    ...
    Flags:
      ...
      - title : t                      (unicode)       [create]
          Set title text. The default value under writemode is Save As. The defaule value under readmode is Open.
          Flag can have multiple arguments, passed either as a tuple or a list.
...
def getInputDeviceRange(*args, **kwargs):
    ...
    Flags:
      - maxValue : max                 (bool)          [create]
          list only the maximum value of the axis                   Flag can have multiple arguments, passed either as a tuple or
          a list.
...
def getModifiers(*args, **kwargs):
    ...
    the modifier's corresponding bit value in the return value. Shift is bit 1, Caps Lock is bit 2, Ctrl is bit 3, and Alt
    is bit 4. See the provided example for more details on testing for each modifier's bit value.
...
def hardware(*args, **kwargs):
    ...
    Flags:
      ...
      - numProcessors : npr            (bool)          [create]
          Returns string identifying the number of processors                       Flag can have multiple arguments, passed
          either as a tuple or a list.
...
def imfPlugins(*args, **kwargs):
    ...
    Flags:
      ...
      - multiFrameSupport : mfs        (unicode)       [create,query]
          multi frame IO is supported                                       Flag can have multiple arguments, passed either as a
          tuple or a list.
...
def iterReferences(parentReference=None, recursive=False, namespaces=False, refNodes=False, references=True, recurseType='depth'):
    """
...
def launch(*args, **kwargs):
    """
    Launch the appropriate application to open the document specified. This command works only on the Macintosh and Windows
    platforms.
...
def launchImageEditor(*args, **kwargs):
    ...
    Flags:
      ...
      - viewImageFile : vif            (unicode)       [create]
          Opens up an Image editor to view images.                          Flag can have multiple arguments, passed either as a
          tuple or a list.
...
def listDeviceAttachments(*args, **kwargs):
    ...
    Flags:
      ...
      - file : f                       (unicode)       [create]
          Specify the name of the file to write out device attachments.                     Flag can have multiple arguments,
          passed either as a tuple or a list.
...
def loadPlugin(*args, **kwargs):
    ...
    Mac OS X. MAYA_PLUG_IN_PATH will be searched looking for a file with the given name.  If you specified the plug-in
    without an extension, the appropriate one for the platform will be automatically appended. When the plug-in is loaded,
    the name used in Maya's internal plug-in registry for the plug-in information will be the file name with the extension
    removed.  For example, if you load the plug-in newNode.mllthe name used in the Maya's registry will be newNode.  This
    value as well as that value with either a .so, .mllor .bundleextension can be used as valid arguments to either the
    unloadPlugin or pluginInfo commands.
    
    Flags:
      - addCallback : ac               (callable)      [create]
      ...
      - removeCallback : rc            (callable)      [create]
          Removes a procedure which was previously added with -addCallback.                         Flag can have multiple
          arguments, passed either as a tuple or a list.
...
def mouse(*args, **kwargs):
    ...
    Flags:
      ...
      - mouseButtonTrackingStatus : mbs (bool)          [create]
          returns the current number of mouse buttons being tracked.                        Flag can have multiple arguments,
          passed either as a tuple or a list.
...
def namespace(*args, **kwargs):
    """
    This command allows a namespace to be created or set or removed. A namespace is a simple grouping of objects under a
    given name. Namespaces are primarily used to resolve name-clash issues in Maya, where a new object has the same name as
    an existing object (from importing a file, for example).  Using namespaces, you can have two objects with the same name,
    as long as they are contained in differenct namespaces. Namespaces are reminiscent of hierarchical structures like file
    ...
    using the add/addNamespace flag. They are created under the current namespace. Changing the current namespace is done
    with -set/setNamespace flag. To reset the current namespace to the root namespace, use 'namespace -set :;'. Whenever an
    object is created, it is added to the current namespace. The relativeNamespace flag can be used to change the way node
    names are displayed in the UI and returned by the 'ls' command. Here are some specific details on how the return from
    the 'ls' command works in relativeNamespace mode: List all mesh objects in the scene:ls -type mesh;The above command
    lists all mesh objects in the root and any child namespaces. In relative name lookup mode, all names will be displayed
    relative to the current namespace. When not in relative name lookup mode (the default behaviour in Maya), results are
    printed relative to the root namespace. Using a \*wildcard:namespace -set myNS;ls -type mesh \*;In relative name lookup
    mode, the \*will match to the current namespace and thus the ls command will list only those meshes defined within the
    current namespace (i.e. myNs). If relative name lookup is off (the default behaviour in Maya), names are root-relative
    and thus \*matches the root namespace, with the net result being that only thoses meshes defined in the root namespace
    will be listed. You can force the root namespace to be listed when in relative name lookup mode by specifying :\*as your
    search pattern (i.e. ls -type mesh :\*which lists those meshes defined in the root namespace only). Note that you can
    also use :\*when relative name lookup mode is off to match the root if you want a consistent way to list the root.
    Listing child namespace contents:ls -type mesh \*:\*;For an example to list all meshes in immediate child namespaces,
    use \*:\*. In relative name lookup mode \*:\*lists those meshes in immediate child namespaces of the current namespaces.
    When not in relative name lookup mode, \*:\*lists meshes in namespaces one level below the root. Recursive listing of
    namespace contents:Example: ls -type mesh -recurse on \*The -recurse flag is provided on the lscommand to recursively
    travese any child namespaces. In relative name lookup mode, the above example command will list all meshes in the
    current and any child namespaces of current. When not in relative name lookup mode, the above example command works from
    the root downwards and is thus equivalent to ls -type mesh.
    ...
    Flags:
      ...
      - moveNamespace : mv             (unicode, unicode) [create]
          ...
          move a namespace containing referenced nodes will result in an error - use the 'file -e -namespace' command to change a
          reference namespace. If there are objects in the source namespace with the same name as objects in the destination
          namespace, an error will be issued. Use the '-force' flag to override this error - name clashes will be resolved by
      ...
      - recurse : r                    (bool)          [create]
          Can be used with the -exists flag to recursively look for the specified namespace                         Flag can have
          multiple arguments, passed either as a tuple or a list.
    
      - relativeNames : rel            (bool)          [create]
...
def namespaceInfo(*args, **kwargs):
    """
    This command displays information about the current namespace. A namespace is a simple grouping of objects under a given
    name. Each item in a namespace can then be identified by its own name, along with what namespace it belongs to.
    Namespaces can contain other namespaces like sets, with the restriction that all namespaces are disjoint. Namespaces are
    primarily used to resolve name-clash issues in Maya, where a new object has the same name as an existing object (from
    importing a file, for example). Using namespaces, you can have two objects with the same name, as long as they are
    contained in differenct namespaces. Note that namespaces are a simple grouping of names, so they do not effect
    selection, the DAG, the Dependency Graph, or any other aspect of Maya.  All namespace names are colon-separated.
    ...
    Flags:
      ...
      - dagPath : dp                   (bool)          [create]
          This flag modifies the '-ls/-listNamespace' and '-lod\-listOnlyDependencyNodes flags to indicate that the names of any
          dag objects returned will include as much of the dag path as is necessary to make the names unique. If this flag is not
          present, the names returned will not include any dag paths.                        Flag can have multiple arguments,
          passed either as a tuple or a list.
      ...
      - recurse : r                    (bool)          [create]
          Can be specified with -listNamespace, -listOnlyNamespaces or -listOnlyDependencyNode to cause the listing to recursively
          include any child namespaces of the current namespaces.
...
def ogs(*args, **kwargs):
    ...
    Flags:
      - reset : r                      (bool)          [create,query]
          ...
          be affected is returned but the reset is not actually done.                                       Flag can have multiple
          arguments, passed either as a tuple or a list.
...
def openGLExtension(*args, **kwargs):
    ...
    Flags:
      ...
      - vendor : vnd                   (bool)          [create]
          Specifies to query the company responsible for the OpenGL implementation.                                         Flag
...
def openMayaPref(*args, **kwargs):
    ...
    Flags:
      - errlog : el                    (bool)          [create,query,edit]
          ...
          called the failing method. Defaults to false(off).                    Flag can have multiple arguments, passed either as
          a tuple or a list.
...
def pluginInfo(*args, **kwargs):
    ...
    Flags:
      - activeFile : af                (bool)          [query]
          ...
          For use during export selected.                   Flag can have multiple arguments, passed either as a tuple or a list.
      ...
      - changedCommand : cc            (callable)      [create]
          adds a callback that will get executed every time the plugin registry changes.  Any other previously registered
      ...
      - unloadOk : uo                  (bool)          [query]
          returns a boolean that specifies whether or not the plugin can be safely unloaded.  It will return false if the plugin
          is currently in use.  For example, if the plugin adds a new dependency node type, and an instance of that node type is
...
def preloadRefEd(*args, **kwargs):
    ...
    Flags:
      ...
      - selectFileNode : sf            (bool)          [query]
          ...
          an argument to the selLoadSettings command.                      Flag can have multiple arguments, passed either as a
          tuple or a list.
...
def projFileViewer(*args, **kwargs):
    """
    This command creates a viewer that lists the project files in the current working directory. The viewer can then be
    attached to a pane. In query mode, return type is based on queried flag.
    ...
    Flags:
      ...
      - deselectIndexedItem : dii      (int)           [create,query,edit]
          Deselect the given index item.
    
      - deselectItem : di              (unicode)       [create,query,edit]
          Deselect the given item.
    
      - displayHidden : dh             (bool)          [create,query,edit]
          Indicates whether hidden files should be displayed or not.                        Flag can have multiple arguments,
          passed either as a tuple or a list.
    
      - displayMode : dm               (int)           [create,query,edit]
          Set the display mode. Default is icon. Mode 0 displays as text. Mode 1 displays as large icons. Mode 2 displays as small
          icons.
      ...
      - numberOfItem : ni              (bool)          [query]
          Return the number of items.
    
      - numberOfSelectedItem : nsi     (bool)          [query]
      ...
      - sortBy : sb                    (unicode)       [create,query,edit]
          Specifies how the entry should be sorted.  Possible values are name, date, and size.
    
      - updateCommand : uc             (script)        [create,edit]
          Specify the update command.
    
      - useTemplate : ut               (unicode)       []
    
      - workspace : ws                 (unicode)       [create,query,edit]
...
def recordAttr(*args, **kwargs):
    ...
    attributes of the node are recorded. In query mode, return type is based on queried flag.
    ...
    Flags:
      ...
      - delete : d                     (bool)          [create]
          Do not record the specified attributes                    Flag can have multiple arguments, passed either as a tuple or
          a list.
...
def referenceEdit(*args, **kwargs):
    ...
    Flags:
      ...
      - changeEditTarget : cet         (unicode, unicode) [create]
          ...
          attribute name (.attr). If an edit currently affects the old target, it will be changed to affect the new target. You
          should use 'referenceQuery' to determine the format of the edit targets. As an example most edits store the long name of
          the attribute (e.g. translateX), so when specifying the old target, a long name must also be used. If the short name is
          specified (e.g. tx), chances are the edit won't be retargeted.
...
def referenceQuery(*args, **kwargs):
    ...
    Flags:
      ...
      - shortName : shn                (bool)          [create]
          This flag modifies the '-f/-filename' flag to indicate that the file name returned will be the short name (i.e. just a
          file name without any directory paths). If this flag is not present, the full name and directory path will be returned.
...
def saveImage(*args, **kwargs):
    ...
    Flags:
      ...
      - sceneFile : sf                 (unicode)       [edit]
          The name of the file that the icon is to be associated with.                      Flag can have multiple arguments,
          passed either as a tuple or a list.
...
def sceneUIReplacement(*args, **kwargs):
    ...
    Flags:
      ...
      - update : u                     (unicode)       [create]
          ...
          the main window pane layout holding the panels.
...
def scriptNode(*args, **kwargs):
    """
    scriptNodes contain scripts that are executed when a file is loaded or when the script node is deleted. The scriptNode
    command is used to create, edit, query, and test scriptNodes. In query mode, return type is based on queried flag.
...
def selLoadSettings(*args, **kwargs):
    ...
    Flags:
      ...
      - numSettings : ns               (int)           [create,query]
          ...
          scene plus 1.                                     Flag can have multiple arguments, passed either as a tuple or a list.
...
def setInputDeviceMapping(*args, **kwargs):
    ...
    Flags:
      ...
      - world : w                      (bool)          [create]
          translate the device coordinates into world space coordinates                     Flag can have multiple arguments,
          passed either as a tuple or a list.
...
def shotTrack(*args, **kwargs):
    ...
    Flags:
      ...
      - numTracks : nt                 (int)           [query]
          To query the number of tracks                                     Flag can have multiple arguments, passed either as a
          tuple or a list.
...
def sysFile(*args, **kwargs):
    ...
    Flags:
      ...
      - removeEmptyDir : red           (bool)          [create]
          ...
          which is not empty.                   Flag can have multiple arguments, passed either as a tuple or a list.
...
def timerX(*args, **kwargs):
    ...
    Flags:
      - startTime : st                 (float)         [create]
          When this flag is used, the command returns the elapsed time since the specified start time.                      Flag
          can have multiple arguments, passed either as a tuple or a list.
...
def translator(*args, **kwargs):
    ...
    Flags:
      ...
      - filter : f                     (bool)          [query]
          Returns the filter string used for this translator.                       Flag can have multiple arguments, passed
          either as a tuple or a list.
      ...
      - objectType : ot                (bool)          [query]
          Returns a list of the file types that can be handled by this translator.
...
def unassignInputDevice(*args, **kwargs):
    ...
    Flags:
      - clutch : c                     (unicode)       [create]
          Only delete command attachments with this clutch.                         Flag can have multiple arguments, passed
          either as a tuple or a list.
...
def undoInfo(*args, **kwargs):
    ...
    Flags:
      ...
      - stateWithoutFlush : swf        (bool)          [create,query]
          Turns undo/redo on or off without flushing the queue.                     Flag can have multiple arguments, passed
          either as a tuple or a list.
...
def unloadPlugin(*args, **kwargs):
    ...
    Flags:
      ...
      - force : f                      (bool)          [create]
          ...
          behave properly.  It will, however, still save properly.
    
      - removeCallback : rc            (callable)      [create]
          Remove a procedure which was previously added with -addCallback.                          Flag can have multiple
          arguments, passed either as a tuple or a list.
